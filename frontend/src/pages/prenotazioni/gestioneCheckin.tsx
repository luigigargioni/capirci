...
const GestioneCheckin = () => {
  ...
  const [appliedDateFromFilter, setAppliedDateFromFilter] =
    useState<Dayjs | null>(dayjs().subtract(1, 'day'))
  ...
  const handleDetail = (id: number) => {
    dispatch(activeItem(''))
    navigate(`/prenotazione/${id}?type=detailCheckin`)
  }

  const handleDelete = (id: number) => {
    fetchApi({
      mod: endpoints.reservation.disable.mod,
      fnz: endpoints.reservation.disable.fnz,
      body: { id },
  ...
  const handleCheckIn = (id: number) => {
    dispatch(activeItem(''))
    navigate(`/prenotazione/${id}?type=checkin`)
  }
  ...
  const columns: TableColumnsType<PrenotazioniListType> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Button onClick={() => handleCheckIn(record.id)}>Check in</Button>
          <Popconfirm
            title="Eliminare?"
  ...
  const handleAddReservation = () => {
    dispatch(activeItem(''))
    navigate('/prenotazione/add?type=detailCheckin')
  }
